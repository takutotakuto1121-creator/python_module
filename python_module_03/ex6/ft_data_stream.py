...
def main():
    ...
    players_capitalized = [
        upper_map[name[0]] + name[1:] if name != "" and 'a' <= name[0] <= 'z' else name
    ...
    score_dict = {name: random.randint(0, 1000) for name in players_capitalized}
    ...
    dict_high = {key: score_dict[key] for key in score_dict if score_dict[key] > ave}
